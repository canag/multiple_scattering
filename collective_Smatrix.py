...
def matrix_Sdip(pos, alpha, lmax):
	...
	in spherical modes up to l=lmax
	...
	# number of spherical modes to be considered
	Nsph = 2*lmax*(2*lmax+1)

	# collective D matrix for N dipolar scatterers
	D = matrix_Ddip(pos, alpha, lmax)
	
	# collective S matrix for N dipolar scatterers
	S = np.eye(Nsph) + 2*D
	return S
